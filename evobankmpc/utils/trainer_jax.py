...
class Trainer():
    ...
    def save_state(self, state, info=None, save_name='best', path=""):
        ...
        save_path = path + self.exp_name + '_model_' + save_name
        self.orbax_checkpointer.save(save_path, ckpt, save_args=save_args, force=True)
    ...
    def step(self, state, info):
        ...
        if self.best_fn is not None:
            current_score = self.best_fn(info)
            best_score = self.best_fn(self.best_info)
