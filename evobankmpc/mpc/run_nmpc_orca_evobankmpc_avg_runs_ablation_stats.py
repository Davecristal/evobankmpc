""" evobankmpc average-runs version.
    ...
    - Adds ablation switches for online bank update, anchor bank, Top-K parent,
      switch hysteresis, delayed update, curvature-aware lateral penalty, and Top-K mu aggregation
"""
...
# =========================
# Ablation switches
# Default setting means full EvoBank-MPC. Add one switch at a time for ablation.
# =========================
parser.add_argument('--disable_adaptive_bank', action='store_true',
                    help='Ablation: disable online adaptive model-bank update')
parser.add_argument('--disable_anchor_bank', action='store_true',
                    help='Ablation: disable anchor-bank preservation')
parser.add_argument('--single_best_parent', action='store_true',
                    help='Ablation: use single best parent instead of Top-K parent resampling')
parser.add_argument('--disable_switch_hysteresis', action='store_true',
                    help='Ablation: switch immediately to the current best model')
parser.add_argument('--disable_delayed_update', action='store_true',
                    help='Ablation: disable delayed/aggressive update schedule around friction change')
parser.add_argument('--disable_curvature_ey', action='store_true',
                    help='Ablation: use constant ey_weight instead of curvature-adaptive ey_weight')
parser.add_argument('--single_model_mu', action='store_true',
                    help='Ablation: estimate mu from the selected control model instead of Top-K aggregation')
...
# Edit this part exactly like your original script before running.
# =========================
# Option 1: ETHZ
# TRACK_NAME = 'ETHZ'
# SIM_TIME = 36.0
...
SIM_TIME = 24.0
# const_decay: 4.8-5.4; sudden5s: 4.8-5.3; sudden10s: 9.7-10.4
FRICTION_START = 9.7
FRICTION_END = 10.4
...
for TRYING in range(args.n_runs):
    # run_seed = args.seed + TRYING
    # print(f"================ RUN {TRYING+1}/{args.n_runs} | seed={run_seed} ================")
    ...
    ENABLE_ADAPTIVE_BANK = not args.disable_adaptive_bank
    ...
    ANCHOR_RATIO = 0.0 if args.disable_anchor_bank else 0.4
    TOPK_PARENT = 1 if args.single_best_parent else 5
    SWITCH_MARGIN = 0.93        # new model must be significantly better
    SWITCH_CONFIRM_STEPS = 1 if args.disable_switch_hysteresis else 3
    ...
    def update_model_bank(current_time_step, avg_errors_phys, current_model_idx, friction_active,
        ...
        global MODEL_BANK, MODEL_PARAMS, model_params_array, MODEL_PARAMS_ARRAY, error_windows_phys, update_restore_counter, HYSTERESIS_STEPS, bank_update_count, bank_replace_total
        ...
        bank_update_count += 1
        bank_replace_total += len(replace_indices)
    ...
    for idt in range(n_steps - horizon):
        ...
        if args.disable_curvature_ey:
            ey_weight = args.ey_weight_base
        else:
        ...
        if idt > 0:
            ...
            if window_count_phys >= LookBack_W:
                ...
                if args.disable_switch_hysteresis:
                    # Ablation: no margin and no confirmation. Directly use the current best model.
            ...
            if ENABLE_ADAPTIVE_BANK and window_count_phys >= LookBack_W and idt > LookBack_W:
                ...
                # Full method: observe after friction change before aggressive bank updates.
                # Ablation --disable_delayed_update: use the ordinary update interval/std immediately.
                if not args.disable_delayed_update:
        ...
        if idt <= LookBack_W:
            ...
            MUs.append((model.Df + model.Dr) / (9.81 * params_veh['mass']))
            if args.single_model_mu:
                # Ablation: planner friction estimate comes only from the selected control model.
                best_Dr = MODEL_BANK[current_model_idx].Dr
                best_Df = MODEL_BANK[current_model_idx].Df
            elif window_count_phys >= LookBack_W:
    ...
    results = {
        ...
        'model_switch_count': int(len(model_switches)),
        'bank_update_count': int(bank_update_count),
        'bank_replace_total': int(bank_replace_total),
    }
...
def summarize_per_run_results(all_run_results):
    """Compute mean/std/worst-run statistics from per-run outputs.

    The summary is intentionally computed after all runs, so the simulator loop
    remains unchanged and all plotting can be done from the saved JSON file.
    """
    if len(all_run_results) == 0:
        return {}

    def arr(key, default=0.0):
        return np.array([float(r.get(key, default)) for r in all_run_results], dtype=float)

    def lap_arr(lap_idx):
        vals = []
        for r in all_run_results:
            laps = r.get('lap_times', [])
            vals.append(float(laps[lap_idx]) if len(laps) > lap_idx else np.nan)
    ...
    def metric_summary(values):
        ...
        if finite.size == 0:
            return {
                ...
                'median': None, 'worst_run_id': None, 'worst_value': None,
            }
    ...
    lap2 = lap_arr(1)
    lap3 = lap_arr(2)
    total_laps = arr('total_lap_time')
    violations = arr('violation_time')
    deviations = arr('mean_deviation')
    costs = arr('mean_cost')
    switch_counts = arr('model_switch_count')
    update_counts = arr('bank_update_count')
    replace_totals = arr('bank_replace_total')

    # Two useful definitions of "worst": safety-worst and lap-time-worst.
    worst_violation_idx = int(np.nanargmax(violations))
    worst_lap_idx = int(np.nanargmax(total_laps))

    summary = {
        'lap1': metric_summary(lap1),
        'lap2': metric_summary(lap2),
        'lap3': metric_summary(lap3),
        'total_lap_time': metric_summary(total_laps),
        'violation_time': metric_summary(violations),
        'mean_deviation': metric_summary(deviations),
        'mean_cost': metric_summary(costs),
        'model_switch_count': metric_summary(switch_counts),
        'bank_update_count': metric_summary(update_counts),
        'bank_replace_total': metric_summary(replace_totals),
        'worst_violation_run_id': worst_violation_idx,
        'worst_violation_run': all_run_results[worst_violation_idx],
        'worst_lap_time_run_id': worst_lap_idx,
        'worst_lap_time_run': all_run_results[worst_lap_idx],
    }
    return summary
...
summary_stats = summarize_per_run_results(all_run_results)
print("========== 10-run Summary Stats =========")
...
if args.output_file:
    out = {
        ...
        'n_models': int(N_MODELS) if 'N_MODELS' in globals() else None,
        'friction_start': float(FRICTION_START),
        'friction_end': float(FRICTION_END),
        'ablation_config': {
            'disable_adaptive_bank': bool(args.disable_adaptive_bank),
            'disable_anchor_bank': bool(args.disable_anchor_bank),
            'single_best_parent': bool(args.single_best_parent),
            'disable_switch_hysteresis': bool(args.disable_switch_hysteresis),
            'disable_delayed_update': bool(args.disable_delayed_update),
            'disable_curvature_ey': bool(args.disable_curvature_ey),
            'single_model_mu': bool(args.single_model_mu),
        },
        'all_trys': ALL_TRYS,
        'avg_vector': ALL_TRYS_MEAN.tolist(),
        'per_run_results': all_run_results,
        'summary_stats': summary_stats,
        'avg_model_switch_count': float(np.mean([r.get('model_switch_count', 0) for r in all_run_results])) if all_run_results else 0.0,
        'avg_bank_update_count': float(np.mean([r.get('bank_update_count', 0) for r in all_run_results])) if all_run_results else 0.0,
        'avg_bank_replace_total': float(np.mean([r.get('bank_replace_total', 0) for r in all_run_results])) if all_run_results else 0.0,
    }
