...
import numpy as np
import casadi as cs
from evobankmpc.models.model import Model
from evobankmpc.params import F110
import jax
...
class Dynamic(Model):
	...
	def _diffequation_jax(self, state, input, params):
		"""
		JAX 版本的微分方程，用于批量计算。
		state: (6,) 状态 [x, y, psi, vx, vy, omega]
		input: (2,) 控制 [pwm, steer]
		params: 参数数组 (Bf, Cf, Df, Br, Cr, Dr)
		"""
		# 解包参数
		...
		# 防止低速数值问题（仅限 vx，与原始 _diffequation 一致）
		vmin = 0.05
		vx = jnp.where(vx < vmin, vmin, vx)
		# 注意：原始 _diffequation 中没有将 vy, omega, steer 置零
		# 这里也不做强制置零，以保持数值行为一致
